...
class PictureDisplay:
    ...
    def play_video(self, path: Path, stop_event: threading.Event) -> bool:
        """Play a saved video once; return True when the user requests exit."""
        with av.open(str(path)) as container:
            stream = container.streams.video[0]
            stream.thread_type = "AUTO"
            frame_rate = (
                float(stream.average_rate) if stream.average_rate is not None else 24.0
            )
            ...
            for frame_index, frame in enumerate(container.decode(stream)):
                ...
                while True:
                    remaining = playback_started + target_offset - time.monotonic()
                    if remaining <= 0:
                        break
                    if stop_event.is_set():
                        return False
